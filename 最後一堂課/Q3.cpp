...
int main()
{
    ...
    // 依照題目範例格式輸出
    cout << sphere.toString() << ":(" << sphere.getRadius() << ")," << setprecision(1) << fixed << sphere.getVolume() << "," << sphere.getColor() << endl;
    cout << cuboid.toString() << setprecision(0) << fixed << ":(" << cuboid.getLength() << "," << cuboid.getWidth() << "," << cuboid.getHeight() << ")," << fixed << setprecision(1) << cuboid.getVolume() << "," << cuboid.getColor() << endl;
