...
{
    out.write(reinterpret_cast<char *>(&(*this)), sizeof(Teacher));
}
// 讀取檔案函式實作
void Teacher::readFromFile(istream &in)
{
    Teacher temp;
    in.read(reinterpret_cast<char *>(&temp), sizeof(Teacher));
    cout << fixed << setprecision(1) << temp.getName() << " " << temp.getAge() << " " << temp.getScore() << endl;
}
// 主程式
int main()
{
    ...
    // 開 Teacher.txt 檔案 (並以輸出、附加、二進制方式)
    io.open("Teacher.txt", ios::out | ios::app | ios::binary);
    // 利用迴圈將物件資訊寫入檔案
    for (int i = 0; i < 4; i++)
        t[i].writeToFile(io);
    // 關檔
    ...
    // 開 Teacher.txt 檔案 (並以輸入和二進制方式)
    io.open("Teacher.txt", ios::in | ios::binary);
    // 利用迴圈將檔案內資訊讀取出來
    for (int i = 0; i < 4; i++)
        t[i].readFromFile(io);
    // 關檔
